...
svn co http://svn.openfoundry.org/openfoundry/trunk/services/vcs "${VCS_CHECKOUT}"
svn co http://svn.openfoundry.org/openfoundry/trunk/openfoundry "${OPENFOUNDRY_PM_CHECKOUT}"
...
( cd /usr/ports/security/pwauth ; make patch ; patch $WRKDIRPREFIX/usr/ports/security/pwauth/work/pwauth-*/config.h < "${OPENFOUNDRY_CHECKOUT}/usr/ports/security/pwauth/config.h.diff" ; make install )
...
csup -g -L 2 "${OPENFOUNDRY_CHECKOUT}/usr/src/for-cvs-supfile"
patch /usr/src/contrib/cvs/src/server.c < "${OPENFOUNDRY_CHECKOUT}/usr/src/contrib/cvs/src/server.c.diff"
...
ln -sf "${VCS_CHECKOUT}/usr/local/openfoundry/etc/openfoundry.conf.dist" "${OPENFOUNDRY_ETC}"
ln -sf "${VCS_CHECKOUT}/usr/local/openfoundry/etc/openfoundry_root.conf.dist" "${OPENFOUNDRY_ETC}"
