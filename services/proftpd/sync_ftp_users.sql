...
#INSERT IGNORE tmp_groups SELECT t1.unixname, t1.id, login from `of_development`.roles_users t3 LEFT JOIN `of_development`.roles t2 ON t3.role_id=t2.id LEFT JOIN `of_development`.projects t1 ON t2.authorizable_id=t1.id LEFT JOIN `of_development`.users t4 ON t3.user_id=t4.id  WHERE t2.authorizable_type='Project' ORDER BY t1.id, login;
