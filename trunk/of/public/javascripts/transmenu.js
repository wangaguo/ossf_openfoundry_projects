...
TransMenu.itemPadding = 5;                             // ¦氪additional padding around each item
...
{
	var mtm_menu = document.getElementById("mtm_menu");
	mtm_menu.innerHTML = '';
	for(i=0; i<menudata.length; i++)
	{
		mtm_menu.innerHTML += ' <a id="mtm_' + menudata[i]["id"] + '" href="' + menudata[i]["url"] + '" class="" >' + menudata[i]["id"] + '</a>';
	}
}
function CreateMenu()
{
	...
	for(i=0; i<menudata.length; i++)
	{
		CreateSubMenu(menudata[i], ms, 1);
	}
}

function CreateSubMenu(datanode, menunode, type, index)
{
	var j;
	if(datanode["submenu"]+'' != 'undefined')
	{
		if(type == 1)
			var newmenunode = menunode.addMenu(document.getElementById("mtm_" + datanode["id"]));
		else{
			var newmenunode = menunode.addMenu(menunode.items[index]);
			}
		for(j=0; j<datanode["submenu"].length; j++)
		{
			newmenunode.addItem(datanode["submenu"][j]["id"], datanode["submenu"][j]["url"]);
			CreateSubMenu(datanode["submenu"][j], newmenunode, 2, j);
		}
	}
}
...
function init() {
  if (TransMenu.isSupported()) {
		...
		CreateMenu();
		TransMenu.renderAll2();
		TransMenu.initialize();
 	}
}

TransMenu.renderAll2 = function() {
	var aMenuHtml = [];
	for (var i = 0, menu = null; menu = this.registry[i]; i++) {
	        aMenuHtml[i] = menu.toString();
	}
	var div = document.createElement("div");
	div.innerHTML = aMenuHtml.join("");
